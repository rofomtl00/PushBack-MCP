...
- TAX INCENTIVE OPTIMIZATION: has the production modeled incentives across multiple jurisdictions? Look up current film tax credit programs — they change annually. Key jurisdictions to compare: Georgia (transferable credit, no cap), New Mexico (refundable, uplifts for TV/rural), California (largest allocation), New Jersey (extended to 2039), UK (VFX receives enhanced rate), Ireland (tiered by budget), and emerging programs (Saudi Arabia, Abu Dhabi, Iowa, Texas). The production should show a side-by-side comparison of net effective cost per jurisdiction, not just the headline credit rate
- LOCATION ECONOMICS: location choice must be justified by: tax incentive fit, crew availability, stage/facility access, travel costs, weather risk, and union jurisdiction impact. A location with a higher credit but no local crew may cost more in travel/accommodation than a lower-credit location with established infrastructure
- BUDGET STRUCTURE: for features, is the budget in industry-standard format (AICP or EP/Movie Magic top sheet)? Does it break down into above-the-line (talent, director, producers, writers) and below-the-line (crew, equipment, post, music)? Is contingency at least 10%? For micro dramas: budget should show per-episode cost, total series cost, and platform revenue/ad model to calculate breakeven
- MICRO DRAMA ECONOMICS: if producing short-form vertical content, evaluate: per-episode budget (market range is currently well under traditional TV), production cycle (compressed schedules), platform distribution strategy (app-based, social, streaming), monetization model (in-app purchases, ads, brand integration), and union vs non-union implications on schedule
- AWS/CLOUD PRODUCTION: if using cloud infrastructure, evaluate: AWS Deadline Cloud for rendering (vs on-premise render farm cost comparison), cloud workstations for editorial/VFX (latency requirements for real-time work), S3/Glacier for asset management and archive, data transfer costs (egress fees on large VFX files can be significant — calculate before committing), and security/DRM requirements for unreleased content in cloud storage
- COMPLETION BOND: for financed productions, has the bonding company approved: script, budget, schedule, key personnel? Is contingency funded to the bond company's requirements? Who controls the production account? What are the takeover triggers (typically: over budget by X%, behind schedule by Y days)?
- DELIVERY REQUIREMENTS: does the production know the EXACT delivery specifications from every distributor? Theatrical (DCP format), streaming (IMF per platform specs — Netflix, Amazon, Disney+ all have different requirements), international (M&E tracks, dubbing/subtitle deliverables, censorship versions). Missing a delivery spec can delay revenue by months
- SCHEDULING: is the schedule realistic given: cast availability windows, location permits/weather, VFX shot count and complexity pipeline, post-production (edit, sound, music, color, VFX finals, DI), and delivery deadlines? Work backwards from the distribution date — does the math work?
- INSURANCE: Entertainment Package Policy covering: cast insurance (what if the lead actor gets injured?), negative film/faulty stock, equipment, third-party property damage, errors & omissions (E&O for IP clearance), workers' comp, general liability. Look up current minimum coverage requirements for the production's jurisdiction
- DISTRIBUTION WINDOWING: for studio-level productions, what is the windowing strategy? Theatrical → PVOD → streaming → FAST channels. For micro dramas: platform exclusivity period, simultaneous multi-platform release, or territory-by-territory rollout? Each model has different revenue implications
- GREENLIGHT P&L MODEL: every production pitch must include a P&L (profit & loss) projection modeled as a revenue waterfall. The waterfall shows recoupment priority: distribution fees/commissions first, then P&A costs, then senior debt, then gap/mezzanine financing, then equity investors, then profit participants. If the pitch does not show a waterfall, the financial model is incomplete. Verify: does the remaining equity return justify the risk at the projected revenue?
- AICP/EP BUDGET FORMAT: commercial productions use AICP format (16 cost categories A through P). Feature/TV budgets use Entertainment Partners format with above-the-line/below-the-line split. If the budget is in a non-standard format, it cannot be compared against industry benchmarks. Check: is contingency a separate line item (not hidden in other categories)?
- PRODUCTION ACCOUNTING: payroll must be processed through an entertainment payroll company (for union compliance, tax incentive qualification, and workers' comp). Is the production accounting on a recognized platform that tracks qualifying spend for tax incentive claims? Non-compliant payroll processing can disqualify the entire production from tax credits
- MOVIELABS 2030 FRAMEWORK: for cloud-based productions, evaluate against the industry's 10 core principles — assets created/ingested directly into the cloud, applications come to media (not media to applications), software-defined workflows. If the production claims to be "cloud-native" but is actually uploading/downloading between local and cloud, it is not following the framework
- DISTRIBUTION WATERFALL ANALYSIS: trace the revenue flow from every distribution channel (theatrical, PVOD, streaming, FAST, international, home entertainment) through the waterfall to the equity position. At what total gross does the investor break even? What percentage of comparable productions reach that gross? If the break-even requires top-quartile performance, the investment is speculative regardless of the creative
- MICRO DRAMA UNIT ECONOMICS: for short-form vertical content, model the economics per episode AND per series. Revenue sources: in-app purchases (coin/episode unlock), ad revenue (CPM by platform), brand integration fees. Compare against current market benchmarks for the platform. A series that costs more per episode to produce than the average revenue per episode on that platform is structurally unprofitable
- COMPLETION BOND READINESS: has the production package been assessed against standard bonding requirements? The bond company evaluates: script lock (no material rewrites during production), budget adequacy (contingency funded), schedule feasibility (no optimistic scheduling), key personnel approval (director, producer, UPM must be bondable), and chain of title clearance. If any element is not bondable, financing may not close
...
8. ABOVE-THE-LINE BLOAT: if talent costs exceed 40% of total budget on a mid-budget production, the below-the-line is being squeezed. Common result: insufficient VFX budget, rushed post-production, or eliminated contingency. Check the ratio
9. TAX CREDIT QUALIFICATION RISK: the production budgets assume full tax credit qualification, but has anyone verified the SPECIFIC qualifying spend requirements? Minimum spend thresholds, labor residency requirements, and eligible expense categories vary by jurisdiction and change annually. A production that falls below the threshold loses the ENTIRE credit, not a proportional amount
10. MICRO DRAMA REVENUE PROJECTION: if the pitch includes revenue projections, what are the assumptions? In-app purchase conversion rates, ad CPMs by platform, brand integration fees — each must be sourced from current market data, not extrapolated from traditional TV economics. The micro drama market is growing but economics are fundamentally different from traditional series
